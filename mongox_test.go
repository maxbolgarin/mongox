...
func TestIndexAndText(t *testing.T) {
	...
	t.Run("Text", func(t *testing.T) {
		...
		_, err = mongox.FindMany[testEntity](ctx, db.Collection(textCollection), mongox.M{"$text": mongox.M{"$search": "tool"}})
		...
		res, err := mongox.FindMany[testEntity](ctx, db.Collection(textCollection), mongox.M{"$text": mongox.M{"$search": "tool"}})
		if err != nil {
			t.Error(err)
		}
		if len(res) != 2 {
...
func TestInsertFindDelete(t *testing.T) {
	...
	t.Run("FindOne_DeleteOne", func(t *testing.T) {
		entity1 := newTestEntity("1")
		...
		err = db.Collection(findOneCollection).Insert(ctx, newTestEntity("2"), newTestEntity("3"), newTestEntity("4"))
		if err != nil {
			t.Error(err)
		}
	...
	t.Run("Find_DeleteAll", func(t *testing.T) {
		...
		testFind(t, ctx, db, []testEntity{entity3}, mongox.M{"id": mongox.M{mongox.Gt: "2", mongox.Lte: "4"}, "name": entity3.Name})
		...
		if err := db.Collection(findCollection).DeleteMany(ctx, nil); err != nil {
			t.Error(err)
		}
...
func TestMain(m *testing.M) {
	...
	err = pool.Retry(func() error {
		...
		client, err = mongox.Connect(
			...
			mongox.Config{
				AppName: "mongox-test",
				Address: "localhost:" + resource.GetPort("27017/tcp"),
